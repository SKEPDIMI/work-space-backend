...
module.exports = app => {
  ...
  app.put('/api/users', requireAuth, async (req, res) => {
    ...
    */
    ...
    if (newEmail) {
      ...
      let sanitizedEmail = sanitizer.sanitize(newEmail)
  ...
  app.post('/api/spaces', requireAuth, async (req, res) => {
    ...
    if (!title || !description) return res.status(405).json({ message: "Missing field(s)" });
  ...
  app.put('/api/spaces/:id', requireAuth, async (req, res) => {
    let description = typeof(req.body.description) ==='string' && req.body.description.trim().length > 0 ? req.body.description : false;
    ...
    let space = await Space_model.findById(req.params.id);
    
    if (!space) return res.status(404).json({ message: 'Could not find space' });

    space.authChangesToken(req.authToken, async (ok, message) => {
  ...
  app.delete('/api/spaces/:id', requireAuth, async (req, res) => {
    ...
    space.authChangesToken(req.authToken, (ok, message) => {
      if (!ok) return res.status(401).json({ message });
  ...
  app.post('/api/spaces/:id/posts', requireAuth, async (req, res) => {
    let spaceId = req.params.id;
    let title = typeof(req.body.title) ==='string' && req.body.title.trim().length > 0 ? req.body.title : false;
    let body = typeof(req.body.body) ==='string' && req.body.body.trim().length > 0 ? req.body.body : false;

    if (!title || !body) return res.status(405).json({message: 'Missing field(s)'});
  ...
  app.put('/api/posts/:id', requireAuth, async (req, res) => { // TODO
    let postId = req.params.id;

    // check if req.user can edit post

    // make changes to the post here
  ...
  app.post('/api/posts/:id/comment', requireAuth, async (req, res) => {
    let body = typeof(req.body.content) ==='string' ? req.body.content : false;

    let post = await Post_model.findById(req.params.id);
    if (!post) return res.status(404).json({ message: 'Could not find post to update' });

    if (body.length <= 1) return res.status(422).json({ message: 'Comment is too short' });
    if (body.length >= 520) return res.status(422).json({ message: 'Comment is too long' });
    ...
    // lets create a comment model instead
  ...
  app.delete('/api/posts/:id', requireAuth, async (req, res) => {
    ...
    post.authChangesToken(req.authToken, (ok, message) => {
      if (!ok) return res.status(401).json({ message })
