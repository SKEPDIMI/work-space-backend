...
module.exports = app => {
  app.get('/api/users/:id?', (req, res) => {
    ...
    let population = typeof(req.query.population) == 'string' ? req.query.population : false;
  ...
  app.post('/api/users', (req, res) => {
    let avatar = typeof(req.files) == 'object' && req.files[0].fieldname === 'avatar' ? req.files[0] : false;
  ...
  app.put('/api/users', async (req, res) => {
    ...
    let avatar = typeof req.files === 'object' ? req.files[0].fieldname == 'avatar' ? req.files[0] : false : false;
  ...
  app.get('/api/posts', (req, res) => {
    // use params instead
    let postId = typeof(req.query.postId) == 'string' && req.query.postId.length > 2 ? req.query.postId : false;
    let spaceId = typeof(req.query.spaceId) == 'string' && req.query.spaceId.length > 2 ? req.query.spaceId : false;
    let userId = typeof(req.query.userId) == 'string' && req.query.userId.length > 2 ? req.query.userId : false;
    let limit = typeof(req.query.limit) == 'string' && req.query.limit.length > 2 ? req.query.limit : 100;
    let population = typeof(req.query.population) == 'string' ? req.query.population : '';
  ...
  app.post('/api/posts', async (req, res) => {
    let spaceId = typeof(req.body.spaceId) == 'string' && req.body.spaceId.trim().length > 0 ? req.body.spaceId : false;
    let title = typeof(req.body.title) == 'string' && req.body.title.trim().length > 0 ? req.body.title : false;
    ...
    try {
      await newPost.save() // Save the new post that was created
  ...
  app.post('/api/auth', async (req, res) => {
    ...
    helpers.compare(password, user.password, (err, res) => {
      if (!res || err) return res.status(401).json({ message:'Could not authenticate' });

      var token = jwt.sign({ id: userData._id }, jwtKey, {expiresIn: 86400 /* expires in 24 hours */});
  ...
  app.post('/api/verifyEmail', async (req, res) => {
    ...
    if (existingSession) {
      TfaSession_model.findByIdAndRemove(existingTfaSession._id);
    }
  ...
  app.get('*', function (req, res){
    console.log("NO ROUTE LEADS HERE")
