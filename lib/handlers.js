...
handlers.users.post = (data, callback) => {
  ...
  .then(user => {
    ...
    callback(200, { message: 'Created user succesfully', auth_token: auth_token }, 'application/json');
  })
...
handlers._posts.post = async function (data, callback) {
  ...
  let auth_token = typeof(data.headers.authorization) != 'undefined' && data.headers.authorization.trim().length > 0 ? data.headers.authorization : false;
...
handlers._posts.put = async function (data, callback) { // TODO THIS PLS DONT FORGET
