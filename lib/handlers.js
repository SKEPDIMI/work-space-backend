      ...
      User.findOne({'email' : {"$regex" : email, "$options" : "i"} }, 'email password', ( err, userData ) => {
        if (!err && userData){
          helpers.compare(password, userData.password, (err, res) => {
            if (!err && res) {
              if (avatar){
                console.log(avatar) // This needs to be handled!
              }
              if (newEmail) {
...
handlers.spaces = function (data, callback) {
  ...
  handlers._spaces.get = function (data, callback) {
    ...
    let subscribed = typeof(data.headers.subscribed) != 'underfined' && !isNaN(Number(data.headers.subscribed)) ? String(data.headers.subscribed) : false; // This number is turned into a string because thats how its stored on MDB
