...
handlers._users.get = function (data, callback) {
  ...
  if (email) {
    User.findOne({ 'email': {"$regex" : email, "$options" : "i"} }, 'email username bio posts followers following', (err, userData) => {
      if (!userData || err) return callback( 406, {message: 'Could not find user'}, 'application/json')
  
      callback(200, userData, 'application/json')
    });
  } else if (id) {
    User.findOne({ _id: id }, 'username bio posts followers following', (err, userData) => {
      if (!userData || err) return callback( 406, {message: 'Could not find user'}, 'application/json')
...
handlers._users.put = function (data, callback) {
  ...
  .then(async userData => {
    ...
    if (newPassword) userData.password = helpers.hashSync(newPassword);
    
    if (startFollowing) {
      const userData2 = await User.findOne({_id: startFollowing});
    ...
    if (stopFollowing) {
      const userData2 = await User.findOne({_id: stopFollowing});
