                              ...
                              if(!err && userData) {
                                helpers.compare(userToAdd.password, userData.password, (err, res) => {
                                  if (!err && res) {
                                    ...
                                    callback(400, {message:'Could not authenticate user'}, 'application/json')
                                  }
                                })
                              } else {
                                callback(400, {message:'Could not find user'}, 'application/json')
                              }
                            });
                          };
                          ...
                          if (userToRemove) {//SECOND ASYNC CALL
                            User.findOne({email:{"$regex" : userToRemove.email, "$options" : "i"}}, (err, userData) => {
                              if(!err && userData) {
                                ...
                                callback(400, {message:'Could not find user'})
                              }
                            });
                          };
                          console.log('gm')
