...
handlers._users.delete = function (data, callback) {
  let password = typeof(data.body.password) != 'undefined' && data.body.password.trim().length > 4 ? data.body.password : false;
  let id = typeof(data.body.id) != 'undefined' && data.body.id.trim().length > 0 ? data.body.id : false;
  if (!email || !password) return callback(400, { message:'Missing field(s)' }, 'application/json');
...
handlers._posts.post = async function (data, callback) {
  ...
  if (body.length > 5520) return callback(400, {message: 'Body is too long'}, 'application/json');

  let decoded = null;

  try {
    decoded = await jwt.verify(token, config.jwtKey);
    if (!decoded.id) return callback(400, { message: 'You are not logged in' }, 'application/json')
  } catch (error) {
    return callback(400, { message: 'You are not logged in' }, 'application/json')
  }
...
handlers._posts.put = async function (data, callback) { // TODO THIS PLS DONT FORGET
  ...
  if (!token || !postId) return callback(400, { message: 'No token provided' }, 'application/json');
  ...
  if (like) {
    if (!post.likes.includes(decoded.id)) {
      ...
      return callback(400, { message: 'Could not unlike, user is has not liked' }, 'application/json');
    }
  } else if (unlike) {
    if (post.likes.includes(decoded.id)) {
  ...
  if (addComment) {
    if (addComment.length <= 1) return callback(400, { message: 'Comment is too short' }, 'application/json');
    if (addComment.length >= 520) return callback(400, { message: 'Comment is too long' }, 'application/json');
    let commentObj = {
      user: decoded.id,
      creationTime: new Date(),
