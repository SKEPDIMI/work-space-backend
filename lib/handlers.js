...
handlers.users.get = function(data, callback) {
  let id = typeof(data.params.id) == 'string' && data.params.id.trim().length > 0 ? data.params.id : false;
  let population = typeof(data.query.population) == 'string' ? data.query.population : false;

  if (id) {
    User_model.findById(id)
    .select('-password -email')
    .safePopulate(population)
    .then(user => {
      callback(200, user, 'application/json')
    })
    .catch(err => callback(404, { message: 'Could not find user by this id' }, 'application/json')) 
  } else {
    User_model.find({})
    .select('-password -email')
    .safePopulate(population)
    ...
    .catch(err => callback(404, { message: 'Could not find users' }, 'application/json')) 
  }
}
...
handlers.users.post = (data, callback) => {
  ...
  let username = typeof(data.body.username) == 'string' && data.body.username.trim().length > 0 ? data.body.username : false;
  ...
  if (!emailIsValid.response) return callback(422, { message: emailIsValid.message }, 'application/json');
  if (!username) return callback (422, { message: 'Please provide a username' }, 'application/json');
  if (!passwordIsValid.response) return callback(422, { message: passwordIsValid.message }, 'application/json');
  ...
  userData.save()
  .then(user => {
    var auth_token = jwt.sign({ id: user._id }, config.jwtKey, {expiresIn: 86400});
    callback(200, { message: 'Created user succesfully', auth_token: auth_token, id: user._id }, 'application/json');
  })
  .catch(err => callback(409, { message: 'Could not create user - It probably already exists' }, 'application/json'));
};

handlers.users.put = async function (data, callback) {
  let auth_token = typeof(data.headers.authorization) == 'string' && data.headers.authorization.trim().length > 0 ? data.headers.authorization : false;

  let newUsername = typeof(data.body.username) == 'string' ? data.body.username : false;
  let newEmail = typeof(data.body.email) == 'string' ? data.body.email : false;
  let newEmailIsValid = helpers.followsGuidelines('email', newEmail);

  let newPassword = data.body.password != 'undefined' ? data.body.password : false;
  let newPasswordIsValid = helpers.followsGuidelines('password', newPassword);

  let bio = typeof(data.body.bio) == 'string' ? data.body.bio : false;
  let avatar = typeof(data.files[0]) === 'object' ? data.files[0].fieldname == 'avatar' ? data.files[0] : false : false;

  if (!auth_token) return callback(401, { message: 'Must provide auth_token' }, 'application/json');

  let decoded = helpers.decodeToken(auth_token);

  if (!decoded.id) {
    return callback(401, { message: 'Invalid token' }, 'application/json');
  }

  let userData = await User_model.findById(decoded.id);
  if (!userData) return callback(404, { message: 'Could not find user'}, 'application/json');
  ...
  */
  ...
  if (newEmail) {
    if (newEmailIsValid.response) {
      ...
      if (!sanitizedEmail) return callback(422, { message: 'Email must not contain tags' }, 'application/json');

      userData.email = sanitizedEmail
    } else {
      return callback(422, { message: newEmailIsValid.message }, 'application/json')
    }
  }
  ...
  if (newUsername) {
    ...
    if (sanitizedUsername.length == 0) {
      return callback(422, { message: 'Username is too short' }, 'application/json')
    }
  ...
  if (bio) {
    ...
    if (sanitizedBio.length < bio.length) {
      return callback(422, { message: 'Bio cant contain tags' }, 'application/json')
    }
  ...
  if (newPassword) {
    if (newPasswordIsValid.response) {
      ...
      return callback(422, { message: newPasswordIsValid.message }, 'application/json')
    }
  };
...
handlers.users.delete = async function (data, callback) {
  let auth_token = typeof(data.headers.auth_token) == 'string' ? data.headers.auth_token : false;
  if (!auth_token) return callback(422, { message:'Missing auth_token' }, 'application/json');

  /* TODO
    When a user is deleted, remove them from the 
    `following` / `follower` props of other users
    they are present in
  */
  let decoded = helpers.decodeToken(auth_token);
  if (!decoded.id) {
    callback(401, { message: 'Invalid session. Please try logging in again.' }, 'application/json')
  } else {
    User_model
...
handlers.spaces = {};
handlers.spaces.get = function (data, callback) {
  let id = typeof(data.params.id) == 'string' ? data.params.id : false;
  let limit = !isNaN(Number(data.query.limit)) ? data.query.limit : 100;
  let population = typeof(data.query.population) == 'string' ? data.query.population : false;
  ...
  if (id) {
    ...
    .safePopulate(population)
    .then(space => {
      callback(200, space, 'application/json')
    })
    .catch(err => {
      callback(404, { message: 'WorkSpace not found' }, 'application/json')
    });
  } else {
    Space_model.find({})
    .limit(limit)
    ...
    .catch(err => callback(404, { message: 'Could not find spaces' }, 'application/json'));
  }
};

handlers.spaces.post = async function (data, callback) {
  let title = typeof(data.body.title) == 'string' && data.body.title.trim().length > 0 ? data.body.title : false;
  let description = typeof(data.body.description) == 'string' && data.body.description.trim().length > 0 ? data.body.description : false;
  let token = typeof(data.headers.token) == 'string' ? data.headers.auhtorization : false;
  ...
  try {
    ...
    if (!decoded.id) return callback(401, { message: 'Invalid session. Please try logging in again.' }, 'application/json')
  } catch (error) {
    return callback(401, { message: 'Invalid token' }, 'application/json')
  }
  ...
  .then(userData => {
    ...
    .catch(err => {
      if (err) return callback(409, { message: "Space already exists" }, 'application/json')
    })
  })
  .catch(err => {
...
handlers.spaces.put = function (data, callback) {
  let id = typeof(data.params.id) == 'string' && data.params.id.trim().length > 0 ? data.params.id : false;
  let auth_token = (data.headers.authorization) && data.headers.authorization.length > 10 ? data.headers.authorization : false;
  let description = typeof(data.body.description) == 'string' && data.body.description.trim().length > 0 ? data.body.description : false;
  let userToAdd = typeof(data.body.userToAdd) == 'string' ? data.body.userToAdd : false;
  let userToRemove = typeof(data.body.userToRemove) == 'string' ? data.body.userToRemove : false;

  if (!id || !auth_token) return callback(422, { message: 'Missing Fields' }, 'application/json');

  Space_model.findById(id)
  .then(space => {
    space.authChangesToken(token, (res, message) => {
      if (!res) return callback(401, { message }, 'application/json');
...
handlers.spaces.delete = function (data, callback) {
  let id = typeof(data.params.id) == 'string' ? data.params.id : false;
  let auth_token = typeof(data.body.authorization) == 'string' && data.body.authorization.trim().length > 0 ? data.body.authorization : false;

  if (!id || !auth_token) return callback(405, {message:'Missing field(s)'}, 'application/json')
...
handlers.posts.get = function (data, callback) {
  ...
  let population = typeof(data.query.population) == 'string' ? data.query.population : '';
...
handlers.posts.post = async function (data, callback) {
  let spaceId = typeof(data.body.spaceId) == 'string' && data.body.spaceId.trim().length > 0 ? data.body.spaceId : false;
  let title = typeof(data.body.title) == 'string' && data.body.title.trim().length > 0 ? data.body.title : false;
  let body = typeof(data.body.body) == 'string' && data.body.body.trim().length > 0 ? data.body.body : false;
  let auth_token = typeof(data.headers.authorization) == 'string' && data.headers.authorization.trim().length > 0 ? data.headers.authorization : false;

  if (!title || !body || !auth_token || !spaceId) return callback(405, {message: 'Missing field(s)'}, 'application/json');

  if (title.length > 45) return callback(422, {message: 'Title is too long'}, 'application/json');
  if (body.length > 5520) return callback(422, {message: 'Body is too long'}, 'application/json');
  ...
  if (!decoded.id) {
    return callback(401, { message: 'Invalid session. Try logging in again' }, 'application/json');
  }
  ...
  if (!user.verified) return callback(401, { message: 'You need to verify your account to create a posts!' }, 'application/json');

  let space = await Space_model.findOne({_id: spaceId});
  if (!space) return callback(404, { message: 'Could not find space to post in' }, 'application/json');
...
handlers.posts.put = async function (data, callback) { // TODO THIS PLS DONT FORGET
  let postId = typeof(data.body.postId) == 'string'&& data.body.postId.trim().length > 0 ? data.body.postId : false;
  let auth_token = typeof(data.headers.authorization) == 'string' && data.headers.authorization.trim().length > 0 ? data.headers.authorization : false;
  let addComment = typeof(data.body.addComment) == 'string' ? data.body.addComment : false; // This will be moved to its own separate route `/users/:id/comment`

  if (!auth_token || !postId) return callback(401, { message: 'No token provided' }, 'application/json');

  let decoded = helpers.decodeToken(auth_token);

  let post = await Post_model.findOne({_id: postId});
  if (!post) return callback(404, { message: 'Could not find post to update' }, 'application/json');

  if (addComment) {
    if (addComment.length <= 1) return callback(422, { message: 'Comment is too short' }, 'application/json');
    if (addComment.length >= 520) return callback(422, { message: 'Comment is too long' }, 'application/json');
    let commentObj = {
      user: decoded.id,
      creationTime: new Date(),
  ...
  .catch(error => {
    callback(400, { message: 'Could not save changes' }, 'application/json')
  })
};

handlers.posts.delete = function (data, callback) {
  let auth_token = typeof(data.headers.authorization) == 'string' && data.headers.authorization.length > 10 ? data.headers.authorization : false;
  let id = typeof(data.params.id) == 'string' && data.params.id.length > 5 ? data.params.id : false;

  if (!id || !auth_token) return callback(422, {message: 'Missing field(s)'}, 'application/json');
...
handlers.auth.get = function (data, callback) {
  let auth_token = typeof(data.headers.authorization) == 'string' && data.headers.authorization.length > 2 ? data.headers.authorization : false;

  if (!auth_token) return callback(405, { message:'No auth_token provided' }, 'application/json')

  jwt.verify(auth_token, config.jwtKey, async (err, decoded) => {
    if (err || !decoded) return callback(401, { message: 'This session is not valid. Log in again, please.' },'application/json')
...
handlers.auth.post = function (data, callback) {
  ...
  if (!email || !password) return callback(422, {message: 'Missing Field(s)'}, 'application/json');

  User_model.findOne({'email': {"$regex" : email, "$options" : "i"} })
  .then(userData => {
    helpers.compare(password, userData.password, (err, res) => {
      if (!res || err) return callback(401, {message:'Could not authenticate'}, 'application/json')
...
handlers.userImage = function (data, callback) {
  ...
  if (id) {
    ...
    callback(422, { message: 'Missing user id' }, 'application/json')
  }
};

handlers.verifyEmail = {};

handlers.verifyEmail.post = async function (data, callback) {
  let userId = typeof(data.body.userId) != 'undefined' && data.body.userId.trim().length > 0 ? data.body.userId : false;

  if (!userId) return callback(422, {message: 'Missing userId'}, 'application/json');

  User_model.findOne({_id: userId})
  .then(async userData => {
    if (userData.verified) return callback(409, { message: 'This account is already verified' }, 'application/json');
    ...
    .then(existingTfaSession => {
      TfaSession_model
      .findByIdAndRemove(existingTfaSession._id);
    })
    .catch(err => {
...
handlers.verifyEmail.get = async function (data, callback) {
  let id = data.query.id || false;

  if (!id) return callback(422, { message: 'Missing id field' }, 'application/json');
  ...
  if (verificationSession.expiresIn < now) {
    ...
    return callback(401, { message: 'This session has expired' }, 'application/json')
  }
