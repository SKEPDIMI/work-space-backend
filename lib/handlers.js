...
handlers._posts.post = async function (data, callback) {
  ...
  if (body.length > 1520) return callback(400, {message: 'Body is too long'}, 'application/json');
...
handlers._posts.put = async function (data, callback) { // TODO THIS PLS DONT FORGET
  ...
  if (addComment) {
    ...
    if (addComment.length >= 120) return callback(400, { message: 'Comment is too long' }, 'application/json');
    let commentObj = {
      user: decoded.id,
      creationTime: new Date(),
