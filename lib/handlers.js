...
handlers._users.get = function (data, callback) {
  ...
  if (email) {
    ...
    User_model.find({})
    .select('username bio posts followers following')
    .sort('-followers')
    .limit(200)
    .then(users => {
...
handlers._spaces.get = function (data, callback) {
  ...
  if (title) {
    ...
    Space_model.findOne({_id: id}, (err, results) => {
      if (err) return callback(400, {message: 'WorkSpace not found'}, 'application/json')
      callback(200, results, 'application/json')
    })
  } else {
...
handlers._spaces.post = function (data, callback) {
  ...
  /*
    Make sure this user is verified and user JWT instead of comparySync
  */

  if (!title || !description || !id) return callback(405, {message: "Missing field(s)"}, 'application/json')
  ...
  .then(userData => {
    ...
    Space_model.create(spaceData, (err) => {
...
handlers._posts.get = function (data, callback) {
  ...
  if (!postId && !spaceId && !userId) return callback(400, {message: 'Missing field(s)'}, 'application/json');
  ...
  if (postId) {
    ...
    Space_model.find({_id: spaceId})
    .then(space => {
      Post_model.find({space: space._id})
      .sort('+creationTime')
      .sort('+helpful')
      .limit(limit)
...
handlers._posts.post = async function (data, callback) {
  ...
  try {
    ...
    if (!decoded.id) return callback(400, { message: 'Invalid token' }, 'application/json')
  } catch (error) {
  ...
  .then(async user => {
    ...
    let space = await Space_model.find({_id: spaceId});

    if (!space) return callback(400, {message: 'Could not find space'}, 'application/json')
    let newPost = {
      title,
      body,
      author: user._id,
