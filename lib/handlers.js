...
handlers.users.get = function(data, callback) {
  let id = typeof(data.params.id) != 'undefined' && data.params.id.trim().length > 0 ? data.params.id : false;
...
handlers.users.put = async function (data, callback) {
  ...
  let newUsername = typeof(data.body.username) != 'undefined' && sanitizer.sanitize(data.body.username).length > 0 ? sanitizer.sanitize(data.body.username) : false;
  let newEmail = typeof(data.body.email) != 'undefined' ? sanitizer.sanitize(data.body.email) : false;
  let newEmailIsValid = helpers.followsGuidelines('email', newEmail);

  let newPassword = typeof(data.body.password) != 'undefined' ? sanitizer.sanitize(data.body.password) : false;
  let newPasswordIsValid = helpers.followsGuidelines('password', newPassword);

  let bio = typeof(data.body.bio) != 'undefined' && sanitizer.sanitize(data.body.bio).length > 0 ? sanitizer.sanitize(data.body.bio) : false;
  ...
  // These will need a route of their own... 
  ...
  if (!auth_token) return callback(406, { message: 'Must provide auth_token' }, 'application/json')
  ...
  */
  ...
  if (newEmail) {
    if (newEmailIsValid.response) {
      userData.email = sanitizer.sanitize(newEmail)
    } else {
...
handlers.users.delete = async function (data, callback) {
  let auth_token = typeof(data.headers.auth_token) != 'undefined' ? data.headers.auth_token : false;
...
handlers._spaces.post = async function (data, callback) {
  ...
  let token = typeof(data.headers.token) != 'undefined' ? data.headers.token : false;
...
handlers._spaces.put = function (data, callback) {
  ...
  if (!id || !email || !password) return callback(422, { message: 'Missing Fields' }, 'application/json');
...
handlers._posts.post = async function (data, callback) {
  ...
  let token = typeof(data.headers.token) != 'undefined' && data.headers.token.trim().length > 0 ? data.headers.token : false;
  ...
  try {
    decoded = await jwt.verify(token, config.jwtKey);
    if (!decoded.id) return callback(401, { message: 'You are not logged in' }, 'application/json')
  } catch (error) {
    return callback(401, { message: 'You are not logged in' }, 'application/json')
  }
...
handlers._posts.put = async function (data, callback) { // TODO THIS PLS DONT FORGET
  ...
  let token = typeof(data.headers.token) == 'string' && data.headers.token.trim().length > 0 ? data.headers.token : false;
  let addComment = typeof(data.body.addComment) == 'string' ? data.body.addComment : false;
  let like = typeof(data.body.like) != 'undefined' ? true : false;
  let unlike = typeof(data.body.unlike) != 'undefined' ? true : false;

  if (!token || !postId) return callback(401, { message: 'No token provided' }, 'application/json');
  ...
  if (like) {
    if (!post.likes.includes(decoded.id)) {
      ...
      return callback(400, { message: 'Could not unlike, user has already liked' }, 'application/json');
    }
  } else if (unlike) {
    if (post.likes.includes(decoded.id)) {
      post.likes = post.likes.filter(i => {
        return i !== decoded.id
      })
    } else {
      return callback(400, { message: 'Could not unlike, user has not liked' }, 'application/json');
    }
  }
...
handlers.auth.get = function (data, callback) {
  ...
  jwt.verify(auth_token, config.jwtKey, async (err, decoded) => {
    if (err || !decoded) return callback(406, { message: 'This session is not valid. Log in again, please.' },'application/json')
