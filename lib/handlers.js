...
handlers.users.get = function(data, callback) {
  ...
  let population = typeof(data.headers.population) == 'string' && data.headers.population.trim().length > 0 ? helpers.jsonToObj(data.headers.population) : {};
...
handlers._spaces.get = function (data, callback) {
  let title = typeof(data.query.title) == 'string' && data.query.title.trim().length > 0 ? data.query.title : false;
  let id = typeof(data.query.id) == 'string' && data.query.id.trim().length > 0 ? data.query.id : false;

  if (title) {
    Space_model.find({"title": {"$regex" : title, "$options" : "i"}}, (err, results) => {
      if (err) return callback(404, {message: 'WorkSpace not found'}, 'application/json')
