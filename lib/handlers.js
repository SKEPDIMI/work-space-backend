...
handlers._users.put = async function (data, callback) {
  ...
  let userData = await User_model.findOne({ _id: id });
  if (!userData) return callback(404, { message: 'Could not find user'}, 'application/json');

  let auth = helpers.compareSync(password, userData.password);
  if (!auth) return callback(406, { message:'Authentication failed' }, 'application/json');
  ...
  */
  ...
  if (newUsername) userData.username = sanitizer.sanitize(newUsername)
  
  if (bio) userData.bio = sanitizer.sanitize(bio)
...
handlers._auth.get = function (data, callback) {
  let token = typeof(data.headers.token) == 'string' && data.headers.token.length > 2 ? data.headers.token : false;
